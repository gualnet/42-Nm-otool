...
int		symtab_loop_ppc(t_vault *vault, struct symtab_command *symtab_cmd, struct nlist *nlist)
{
	unsigned int	i;
	int				j;
	char 			*str;
	...
	while (i < swap_endian(symtab_cmd->nsyms))
	{
		if ((str = offset_jumper(vault, vault->strtab, swap_endian(nlist[i].n_un.n_strx))) == NULL)
			return (-1);
...
int		handle_symtab_ppc(t_vault *vault, struct load_command *lc)
{
	...
	vault->strtab = offset_jumper(vault, vault->f_dump, swap_endian(symtab_cmd->stroff));
