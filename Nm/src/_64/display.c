...
/*   Updated: 2018/05/10 18:25:41 by galy             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		lssi_2(t_vault *vault, t_lc_lnk *lc_lnk, struct load_command *lc)
{
	...
	segcmd = (void*)lc;
	if ((seccmd = offset_jumper(vault, lc, \
	sizeof(struct segment_command_64))) == NULL)
		return (-1);
	i = 0;
	while (i < segcmd->nsects)
	{
		...
		if ((seccmd = offset_jumper(vault, seccmd, \
		sizeof(struct section_64))) == NULL)
			return (-1);
		i++;
	}
	return (1);
