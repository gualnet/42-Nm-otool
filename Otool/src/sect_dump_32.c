...
/*   Updated: 2018/04/25 19:00:14 by galy             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		err_msg_print_sect_dump_32(void)
{
	ft_printf("\n\033[31motool error :\n\ttruncated or malformed "
	"object (offset required past the end of the file).\033[0m\n");
	return (-1);
}
...
int		print_sect_dump_32(t_vault *vault)
{
	...
	while (cpt < sect_hdr->size && vault->sect != NULL)
	{
		...
		if ((hex_letter = offset_jumper(vault, vault->sect, cpt)) == NULL)
			return (err_msg_print_sect_dump_32());
